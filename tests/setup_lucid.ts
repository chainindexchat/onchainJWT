...
import { PRIVATE_KEY, SERVER_PRIVATE_KEY, MAESTRO_API_KEY } from "./config.ts";
import { OurEmulator } from "./emulator_provider.ts";

const uidInputData = new TextEncoder().encode(
  PRIVATE_KEY + MAESTRO_API_KEY,
);
...
export const EMULATOR_PRIVATE_KEY = SERVER_PRIVATE_KEY;
...
const testnetConfigured = MAESTRO_API_KEY.length != 0 &&
...
export const lucidTestnet = testnetConfigured
  ...
  : undefined;
lucidTestnet?.selectWalletFromPrivateKey(SERVER_PRIVATE_KEY);
